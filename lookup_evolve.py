...
from evolve_utils import prepare_objective
...
## Todo: make initial sequence evolvable
...
class LookerUpParams(Params):
    ...
    def player(self):
        player = self.PlayerClass(self.table)
        return player

    def copy(self):
        return LookerUpParams(self.plays, self.op_plays, self.op_start_plays,
                              self.initial_actions, self.mutation_rate,
                              dict(self.table))
